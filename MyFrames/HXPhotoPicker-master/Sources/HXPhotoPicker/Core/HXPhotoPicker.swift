...
public enum HX {
    ...
    public enum ImageTargetMode {
        /// The aspect ratio is consistent with the original image, and the height will be calculated based on `targetSize`
        case fill
        /// Stretch/scale based on `targetSize`
        case fit
        /// If the proportion of `targetSize` is different from the original image, it will be displayed in the center.
        case center
    }
}
