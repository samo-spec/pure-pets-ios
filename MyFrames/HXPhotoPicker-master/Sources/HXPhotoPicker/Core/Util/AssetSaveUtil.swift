...
public struct AssetSaveUtil {
    ...
    public enum AlbumType {
        /// Only save to `all albums`, do not create new albums
        case none
        
        // Save to the album with the specified name. If the album does not exist, it will be created first.
        /// Project name
        case displayName
    ...
    /// Save resources to the system album
    /// - Parameters:
    /// - type: save type
    /// - albumType: The type that needs to be saved to the custom album
    /// - creationDate: creation time, default current time
    ...
    /// - creationDate: creation time, default current time
    /// - location: location information
    /// - completion: the result after saving
    public static func save(
        type: SaveType,
        albumType: AlbumType = .displayName,
        creationDate: Date = .init(),
