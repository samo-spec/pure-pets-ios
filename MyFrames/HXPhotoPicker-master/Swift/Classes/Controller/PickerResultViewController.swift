...
class PickerResultViewController: UIViewController,
    ...
    /// Whether the original image is selected
    var isOriginal: Bool = false
    /// Local resources captured by the camera
...
//        config.editor.buttonType = .top
    ...
    @objc func didCancelButtonClick() {
        ...
        PhotoTools.showAlert(
            viewController: self,
            title: "Do you want to keep this edit?",
            message: nil,
            leftActionTitle: "Do not retain",
            leftHandler: { _ in
            self.removeLocalPhotoAssetFile()
            self.dismiss(animated: true, completion: nil)
        }, rightActionTitle: "Keep") { _ in
            do {
    ...
    /// Jump to the resource selection interface
...
//        )
        if #available(iOS 13.0, *) {
            Task {
                for (index, selectedAsset) in selectedAssets.enumerated() {
                    do {
                        print("number" + String(index + 1) + "number")
                        let result: AssetURLResult = try await selectedAsset.object(nil)
                        if let livePhoto = result.livePhoto {
                            print("Picture address in LivePhoto:", livePhoto.imageURL)
                            print("Video address in LivePhoto:", livePhoto.videoURL)
                        }else {
                            print(
                                result.urlType == .network ?
                                result.mediaType == .photo ?
                                "Online picture address:" : "Online video address:" :
                                    result.mediaType == .photo ?
                                "Local image address" : "Local video address",
                                result.url
                            )
                        }
                    } catch {
                        print("Address acquisition failed: ", error)
                    }
                }
                view.hx.hide(animated: false)
                view.hx.showSuccess(text: "Acquisition completed", delayHide: 1.5, animated: true)
            }
        } else {
            selectedAssets.getURLs(
                ...
                self.view.hx.showSuccess(text: "Acquisition completed", delayHide: 1.5, animated: true)
            }
        }
    }
    ...
    /// Jump to separate preview interface
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        HXPhotoPicker.PhotoBrowser.show(
            //Preview resource array
            selectedAssets,
            //The current preview position
            pageIndex: indexPath.item,
            // Preview related configurations
            config: config,
            //The initial UIImage of the transition animation
            transitionalImage: cell?.photoView.image
        ) { index, _ in
            //The corresponding UIView at the beginning/end of the transition
            ...
            PhotoTools.showAlert(
                ...
                title: "Whether to delete the current resource",
                leftActionTitle: "OK",
    ...
    func previewLongPressClick(photoAsset: PhotoAsset, photoBrowser: HXPhotoPicker.PhotoBrowser) {
        let alert = UIAlertController(title: "Long press event", message: nil, preferredStyle: .actionSheet)
        alert.addAction(
            .init(
                title: "Save",
                style: .default,
...
class ResultViewCell: PhotoPickerViewCell {
    ...
    override func requestThumbnailImage() {
        // Because there won’t be many cells here, reset the targetWidth to make the picture clearer.
