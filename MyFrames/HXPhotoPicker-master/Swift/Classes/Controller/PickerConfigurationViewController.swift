...
class PickerConfigurationViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(
            title: showOpenPickerButton ? "Open Picker" : "OK",
            style: .done,
            target: self,
    ...
    func getRowContent(_ rowType: ConfigRowTypeRule) -> String {
        if let rowType = rowType as? ConfigRowType {
            ...
            case .shouldAutorotate:
                return config.shouldAutorotate ? "allow" : "not allowed"
            case .selectOptions:
                return config.selectOptions.title
            case .selectMode:
                return config.selectMode.title
            case .allowSelectedTogether:
                return config.allowSelectedTogether ? "allow" : "not allowed"
            case .allowLoadPhotoLibrary:
                return config.allowLoadPhotoLibrary ? "allow" : "not allowed"
            case .albumShowMode:
                return config.albumShowMode.title
            case .creationDate:
                return config.creationDate ? "Yes" : "No"
            case .reverseOrder:
                return config.photoList.sort == .desc ? "Yes" : "No"
...
extension PickerConfigurationViewController {
    ...
    enum ConfigRowType: String, CaseIterable, ConfigRowTypeRule {
        ...
        var title: String {
            ...
            case .shouldAutorotate:
                return "Allow rotation (valid in full screen)"
            case .selectOptions:
                return "resource type"
            case .selectMode:
                return "select mode"
            case .allowSelectedTogether:
                return "Photos and videos can be selected at the same time"
            case .allowLoadPhotoLibrary:
                return "Allow loading of system photo library"
            case .albumShowMode:
                return "Album display mode"
            case .creationDate:
                return "Sort by creation time"
            case .reverseOrder:
                return "Display in reverse order"
            case .photoSelectionTapAction:
                return "List photo Cell click action"
            case .videoSelectionTapAction:
                return "List video Cell click action"
            case .maximumSelectedPhotoCount:
                return "Maximum number of photos that can be selected"
            case .maximumSelectedVideoCount:
                return "Maximum number of videos that can be selected"
            case .maximumSelectedCount:
                return "The maximum total number that can be selected"
            case .maximumSelectedVideoDuration:
                return "Maximum video selection duration"
            case .minimumSelectedVideoDuration:
                return "Minimum video selection duration"
            case .photoRowNumber:
                return "display quantity per line"
            case .videoPlayType:
                return "video playback type"
            case .addCamera:
                return "Add camera to list"
            }
        }
        var detailTitle: String {
...
extension AlbumShowMode {
    var title: String {
        ...
        case .present:
            return "pop-up window controller"
        }
    }
}
...
extension PhotoPreviewViewController.PlayType {
    var title: String {
        switch self {
        case .normal:
            return "Do not play automatically"
        case .auto:
            return "autoplay"
        case .once:
            return "Autoplay once"
        }
    }
}
