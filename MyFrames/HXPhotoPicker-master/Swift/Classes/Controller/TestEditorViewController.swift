...
class TestEditorViewController: HXBaseViewController {
    ...
    lazy var editorView: EditorView = {
        ...
        // Set margins like this. There will be no shrink/enlarge animation when entering/exiting the editing state.
//        view.contentInset = .init(top: 20, left: 20, bottom: 20 + UIDevice.bottomMargin, right: 20)
        // Set margins like this. Entering/exiting the editing state will have a shrink/enlarge animation.
...
//        view.setRoundMask(animated: false)
    ...
    func showAlert() {
        ...
        if editorView.type != .unknown {
            alert.addAction(.init(title: "Continue editing using editor", style: .default, handler: { [weak self] _ in
                        ...
                        case .success(let result):
                            ...
                            HXPhotoPicker.PhotoBrowser.show(
                                ...
                                photoAsset.saveToSystemAlbum { result in
                                    ...
                                    switch result {
                                    case .success:
                                        photoBrowser.view.hx.showSuccess(text: "Save successfully", delayHide: 1.5, animated: true)
                                    case .failure:
                                        photoBrowser.view.hx.showWarning(text: "Save failed", delayHide: 1.5, animated: true)
                                    }
                                }
                            }
        ...
        if editorView.state == .edit {
            ...
            alert.addAction(.init(title: "旋转", style: .default, handler: { [weak self] _ in
                let alert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
                alert.addAction(.init(title: "Rotate at any angle", style: .default, handler: { [weak self] _ in
                    let textAlert = UIAlertController(title: "Enter the angle of rotation", message: nil, preferredStyle: .alert)
                ...
                alert.addAction(.init(title: "Rotate 90° to the left", style: .default, handler: { [weak self] _ in
                    self?.editorView.rotateLeft(true)
                }))
                alert.addAction(.init(title: "Rotate 90° to the right", style: .default, handler: { [weak self] _ in
            ...
            alert.addAction(.init(title: "镜像", style: .default, handler: { [weak self] _ in
                let alert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
                alert.addAction(.init(title: "Horizontal Mirror", style: .default, handler: { [weak self] _ in
                    self?.editorView.mirrorHorizontally(true)
                }))
                alert.addAction(.init(title: "Vertical Mirror", style: .default, handler: { [weak self] _ in
                    self?.editorView.mirrorVertically(true)
                }))
                alert.addAction(UIAlertAction.init(title: "取消", style: .cancel, handler: nil))
                self?.presendAlert(alert)
            }))
            alert.addAction(.init(title: editorView.isFixedRatio ? "Cancel fixed ratio" : "Fixed ratio", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
            ...
            alert.addAction(.init(title: editorView.isRoundMask ? "Cancellation Round" : "Round Cut", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
            ...
            alert.addAction(.init(title: "Modify cropping box ratio", style: .default, handler: { [weak self] _ in
                let alert = UIAlertController.init(title: "Modify proportion", message: nil, preferredStyle: .alert)
            ...
            alert.addAction(.init(title: editorView.isShowScaleSize ? "Hide scale size" : "Show scale size", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
                self.editorView.isShowScaleSize = !self.editorView.isShowScaleSize
            }))
            alert.addAction(.init(title: "Add Mask", style: .default, handler: { [weak self] _ in
                ...
                maskAlert.addAction(.init(title: "Remove Mask", style: .destructive, handler: { [weak self] _ in
            ...
            alert.addAction(.init(title: "Modify mask type", style: .default, handler: { [weak self] _ in
                let maskAlert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
            ...
            alert.addAction(.init(title: "Confirm Editing", style: .default, handler: { [weak self] _ in
            ...
            alert.addAction(.init(title: "Cancel Editing", style: .default, handler: { [weak self] _ in
            ...
            alert.addAction(.init(title: "Add Sticker", style: .default, handler: { [weak self] _ in
            ...
            if editorView.type == .video {
                alert.addAction(.init(title: "Add Music Stickers", style: .default, handler: { [weak self] _ in
                    ...
                    let alert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
                    let fileNames = ["The beauty of the world is closely related to you"]
            ...
            alert.addAction(.init(title: !editorView.isDrawEnabled ? "Turn on drawing" : "Turn off drawing", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
            ...
            alert.addAction(.init(title: "Painting Settings", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
                let alert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
                alert.addAction(.init(title: "Brush Width", style: .default, handler: { [weak self] _ in
                    guard let self = self else { return }
                    let alert = UIAlertController.init(title: "Brush Width Ratio", message: nil, preferredStyle: .alert)
                    alert.addTextField { (textfield) in
                        textfield.keyboardType = .numberPad
                        textfield.placeholder = "Enter brush width ratio"
                    }
                    alert.addAction(
                        UIAlertAction(
                            title: "OK",
                ...
                alert.addAction(.init(title: "Brush Color", style: .default, handler: { [weak self] _ in
                    guard let self = self else { return }
            ...
            if self.editorView.isCanUndoDraw {
                alert.addAction(.init(title: "Undo the last painting", style: .default, handler: { [weak self] _ in
                    self?.editorView.undoDraw()
                }))
                alert.addAction(.init(title: "Undo all drawings", style: .default, handler: { [weak self] _ in
                    self?.editorView.undoAllDraw()
                }))
            }
            if editorView.type == .image {
                alert.addAction(.init(title: !editorView.isMosaicEnabled ? "Turn on mosaic painting" : "Turn off mosaic painting", style: .default, handler: { [weak self] _ in
                    guard let self = self else { return }
                ...
                alert.addAction(.init(title: "Mosaic smear settings", style: .default, handler: { [weak self] _ in
                    guard let self = self else { return }
                    let alert = UIAlertController.init(title: nil, message: nil, preferredStyle: .actionSheet)
                    alert.addAction(.init(title: "Mosaic Width", style: .default, handler: { [weak self] _ in
                        guard let self = self else { return }
                        let alert = UIAlertController.init(title: "Mosaic Width Ratio", message: nil, preferredStyle: .alert)
                        alert.addTextField { (textfield) in
                            textfield.keyboardType = .numberPad
                            textfield.placeholder = "Enter mosaic width ratio"
                        }
                        alert.addAction(
                            UIAlertAction(
                                title: "OK",
                    ...
                    alert.addAction(.init(title: "Paint width", style: .default, handler: { [weak self] _ in
                        guard let self = self else { return }
                        let alert = UIAlertController.init(title: "width ratio", message: nil, preferredStyle: .alert)
                        alert.addTextField { (textfield) in
                            textfield.keyboardType = .numberPad
                            textfield.placeholder = "Enter the smear width ratio"
                        }
                        alert.addAction(
                            UIAlertAction(
                                title: "OK",
                    ...
                    alert.addAction(.init(title: "Switch to" + (self.editorView.mosaicType == .mosaic ? "Smear" : "Mosaic"), style: .default, handler: { [weak self] _ in
                        guard let self = self else { return }
                ...
                if self.editorView.isCanUndoMosaic {
                    alert.addAction(.init(title: "Undo the last mosaic painting", style: .default, handler: { [weak self] _ in
                        self?.editorView.undoMosaic()
                    }))
                    alert.addAction(.init(title: "Undo all mosaics", style: .default, handler: { [weak self] _ in
            ...
            if editorView.type == .video {
                if !editorView.isVideoPlaying {
                    alert.addAction(.init(title: "Play video", style: .default, handler: { [weak self] _ in
                        self?.editorView.playVideo()
                    }))
                }else {
                    alert.addAction(.init(title: "Pause video", style: .default, handler: { [weak self] _ in
                        self?.editorView.pauseVideo()
                    }))
                }
            }
            alert.addAction(.init(title: "Enter editing mode", style: .default, handler: { [weak self] _ in
                guard let self = self else { return }
        ...
        alert.addAction(.init(title: "Select photos/videos for editing", style: .default, handler: { [weak self] _ in
...
class TestEditorAudioLrc: Equatable, Codable {
    ...
    func parseLrc() {
        ...
        if lyrics.isEmpty {
            lyricIsEmpty = true
            lyrics.append(.init(lyric: "This song has no lyrics yet, please enjoy"))
        }
    }
    ...
    func lyric(
        ...
        if lyricIsEmpty {
            return .init(lyric: "This song has no lyrics yet, please enjoy")
        }
        for lyric in lyrics {
