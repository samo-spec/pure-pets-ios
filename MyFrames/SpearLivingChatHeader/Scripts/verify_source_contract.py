...
checks.append(("header height stays content-owned", "ZStack(alignment: .top)" not in header_source and "headerContent" in header_source and ".background {" in header_source))
...
checks.append(("context persists while trust utilities reveal in one shared surface", "if let context = model.context" in ready_source and "if showsIdentityExpansion" in ready_source and "deckIdentity" not in ready_source and ready_source.count("SpearHeaderDeck(") == 1 and "SpearHeaderDeck(" not in context_view_source and "SpearHeaderDeck(" not in expansion_source and "struct SpearHeaderDeck" in deck_source))
checks.append(("accessibility context and utilities share one bounded scroll owner", ready_source.count("ScrollView(.vertical)") == 1 and "ScrollView(" not in expansion_source and "accessibilityExpansionMaximumHeight" in ready_source and "accessibilityExpansionMaximumHeight" in style_source))
