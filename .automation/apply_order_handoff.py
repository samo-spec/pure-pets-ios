...
path = Path('Pure Pets/MainApp/PAYMENTS/CartAndOrdersFiles/PPOrderDetailsMissionControlViews.swift')
...
old_progress = '''            VStack(spacing: PPSpace.sm) {
...
if old_progress not in source:
    raise SystemExit('Expected legacy hero progress block was not found; refusing to edit.')
source = source.replace(old_progress, '            orderHandoffRail', 1)

marker = '''    private var timelineSection: some View {'''
if marker not in source:
    raise SystemExit('Timeline insertion point not found; refusing to edit.')
