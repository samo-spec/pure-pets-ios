...
/*
 ...
 dispatch_once(&onceToken, ^{
 // Configure Firebase
 [FIRApp configure]; // per Firebase docs:contentReference[oaicite:5]{index=5}
 [FIRFirestore enableLogging:YES];
 [[FIRConfiguration sharedInstance] setLoggerLevel:FIRLoggerLevelMin];
 [FIRFirestore firestore]; // initialize Firestore
 });
 ...
 [Language setLanguage:[Language currentLanguageCode]];
 NSLog(@"[Language][AppDelegate] [Language currentLanguageCode] %@", [Language currentLanguageCode]);
 [[UIView appearance] setSemanticContentAttribute:
 Language.isRTL ? UISemanticContentAttributeForceRightToLeft
 : UISemanticContentAttributeForceLeftToRight];
 
 
 [[UINavigationBar appearance] setSemanticContentAttribute:
 Language.isRTL ? UISemanticContentAttributeForceRightToLeft
 : UISemanticContentAttributeForceLeftToRight];
 
 [[UITabBar appearance] setSemanticContentAttribute:
 Language.isRTL ? UISemanticContentAttributeForceRightToLeft
 : UISemanticContentAttributeForceLeftToRight];
 
 // Enable IQKeyboardManager
 NSLog(@"[AppDelegate] Enabling IQKeyboardManager");
 IQKeyboardManager.sharedManager.enable = YES;
 IQKeyboardManager.sharedManager.enableAutoToolbar = YES;
 IQKeyboardManager.sharedManager.toolbarTintColor = [GM appPrimaryColor];
 
 // Restore Google Sign-In session
 //NSLog(@"[AppDelegate] Restoring previous Google Sign-In session");
 //[GIDSignIn.sharedInstance restorePreviousSignInWithCompletion:^(GIDGoogleUser *user, NSError *error) {
 //    NSLog(@"[Google Sign-In] Restored with accessToken: %@", user.accessToken);
 //}];
 
 // Set up Firebase Auth listener
 NSLog(@"[AppDelegate] Adding Firebase auth state listener");
 __unused FIRAuthStateDidChangeListenerHandle handle =
 [[FIRAuth auth] addAuthStateDidChangeListener:^(FIRAuth *auth, FIRUser *user) {
 if (user) {
 NSLog(@"[Auth] User logged in: %@ (%@)", user.phoneNumber, user.uid);
 [user getIDTokenWithCompletion:^(NSString *token, NSError *error) {
 NSLog(@"[Auth] ID token: %@", token);
 }];
 }
 }];
 
 // Register for push notifications
 NSLog(@"[AppDelegate] Registering for notifications");
 UNUserNotificationCenter *center = [UNUserNotificationCenter currentNotificationCenter];
 center.delegate = self; // receive notifications while app is foreground
 [center requestAuthorizationWithOptions:(UNAuthorizationOptionAlert | UNAuthorizationOptionBadge)
 completionHandler:^(BOOL granted, NSError * _Nullable error) {
 if (granted) {
 dispatch_async(dispatch_get_main_queue(), ^{
 [application registerForRemoteNotifications];
 NSLog(@"[AppDelegate] Remote notification permission granted");
 });
 } else {
 NSLog(@"[AppDelegate] Remote notification permission denied: %@", error);
 }
 }];
 ...
 if (userID) {
 [[FIRCrashlytics crashlytics] setUserID:userID];
 NSLog(@"[AppDelegate] Set Crashlytics user ID: %@", userID);
 }
 [[UIView appearance] setSemanticContentAttribute:
 Language.isRTL ? UISemanticContentAttributeForceRightToLeft
 : UISemanticContentAttributeForceLeftToRight];
 ...
 [PPTouchesHelper start]; // show touches overlay
 ...
 [NSCharacterSet characterSetWithCharactersInString:@"<>"]];
 token = [token stringByReplacingOccurrencesOfString:@" " withString:@""];
 NSLog(@"[AppDelegate] didRegisterForRemoteNotifications, device token: %@", token);
 }
 ...
 // Handle incoming FCM token refresh
 - (void)messaging:(FIRMessaging *)messaging didReceiveRegistrationToken:(NSString *)fcmToken {
 NSLog(@"[FIRMessaging] didReceiveRegistrationToken: %@", fcmToken);
 if (fcmToken && UserManager.sharedManager.currentUser.ID) {
 [UserManager.sharedManager updateCurrentUserWithPPUserTokenID:fcmToken];
 NSLog(@"[AppDelegate] Updated user with new FCM token");
 }
 NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
 [prefs setValue:fcmToken forKey:@"PPUserTokenID"];
 [prefs synchronize];
 // Retrieve an access token from our server if needed
