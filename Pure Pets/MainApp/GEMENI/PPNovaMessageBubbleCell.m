...
- (void)setupUI {
    ...
    [self.bubbleShadowView addSubview:self.bubbleMaterialView];
    ...
    NSLayoutConstraint *avatarEdge = rtl
        ? [self.avatarView.rightAnchor constraintEqualToAnchor:self.contentView.rightAnchor constant:-16.0]
        : [self.avatarView.leftAnchor constraintEqualToAnchor:self.contentView.leftAnchor constant:16.0];

    NSLayoutConstraint *assistantPrimary = rtl
        ? [self.bubbleShadowView.rightAnchor constraintEqualToAnchor:self.avatarView.leftAnchor constant:-8.0]
        : [self.bubbleShadowView.leftAnchor constraintEqualToAnchor:self.avatarView.rightAnchor constant:8.0];
    NSLayoutConstraint *assistantLimit = rtl
        ? [self.bubbleShadowView.leftAnchor constraintGreaterThanOrEqualToAnchor:self.contentView.leftAnchor constant:70.0]
        : [self.bubbleShadowView.rightAnchor constraintLessThanOrEqualToAnchor:self.contentView.rightAnchor constant:-70.0];
    NSLayoutConstraint *userPrimary = rtl
        ? [self.bubbleShadowView.leftAnchor constraintEqualToAnchor:self.contentView.leftAnchor constant:16.0]
        : [self.bubbleShadowView.rightAnchor constraintEqualToAnchor:self.contentView.rightAnchor constant:-16.0];
    NSLayoutConstraint *userLimit = rtl
        ? [self.bubbleShadowView.rightAnchor constraintLessThanOrEqualToAnchor:self.contentView.rightAnchor constant:-70.0]
        : [self.bubbleShadowView.leftAnchor constraintGreaterThanOrEqualToAnchor:self.contentView.leftAnchor constant:70.0];
...
- (void)pp_applyAlignmentForAssistant:(BOOL)assistant maxWidth:(CGFloat)maxWidth {
    ...
    if (assistant) {
        ...
        UIFont *font = self.messageLabel.font ?: [UIFont systemFontOfSize:16.0 weight:UIFontWeightRegular];
