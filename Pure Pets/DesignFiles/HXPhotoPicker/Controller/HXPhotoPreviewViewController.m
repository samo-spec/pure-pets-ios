...
- (void)viewDidAppear:(BOOL)animated {
    ...
    NSInteger realIndex = self.currentModelIndex;
    if ([HXPhotoTools isRTLLanguage]) {
        realIndex = self.modelArray.count - 1 - realIndex;
    }

    HXPhotoModel *model = self.modelArray[realIndex];

    self.currentModel = model;
 

    NSIndexPath *realIP = [NSIndexPath indexPathForItem:realIndex inSection:0];
    HXPhotoPreviewViewCell *cell = (HXPhotoPreviewViewCell *)[self.collectionView cellForItemAtIndexPath:realIP];
...
- (NSMutableArray *)modelArray {
    ...
    - (void)setCurrentModelIndex:(NSInteger)currentModelIndex {
        ...
        // Normalize incoming index from grid (LTR)
        NSInteger normalized = currentModelIndex;

        if ([HXPhotoTools isRTLLanguage]) {
            normalized = self.modelArray.count - 1 - currentModelIndex;
        }

        _currentModelIndex = normalized;
    }
