...
+ (void)requestAuthorization:(UIViewController *)viewController
    ...
    [PPPermissionHelper requestPhotoLibraryPermissionFromViewController:viewController
                                                            completion:^(__unused BOOL granted) {
        PHAuthorizationStatus resolvedStatus = [self authorizationStatus];
        if (handler) handler(resolvedStatus);

        if (resolvedStatus == PHAuthorizationStatusAuthorized) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"HXPhotoRequestAuthorizationCompletion" object:nil];
            return;
        }
#ifdef __IPHONE_14_0
        if (@available(iOS 14, *)) {
            if (resolvedStatus == PHAuthorizationStatusLimited) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"HXPhotoRequestAuthorizationCompletion" object:nil];
            }
        }
#endif
...
+ (void)showUnusableCameraAlert:(UIViewController *)vc {
    [PPPermissionHelper showPermissionDeniedAlertForFeature:kLang(@"pp_perm_camera_feature")
                                           onViewController:vc];
}
+ (void)openSetting {
    [PPPermissionHelper openAppSettings];
}
