...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        NSMutableArray *modelArray = [NSMutableArray arrayWithArray:self.dataList];
        HXPhotoPreviewViewController *vc = [[HXPhotoPreviewViewController alloc] init];
        vc.disableaPersentInteractiveTransition = self.disableaInteractiveTransition;
        vc.outside = YES;
        vc.manager = self.manager;
        ...
        vc.modelArray = self.dataList;
        vc.currentModelIndex =  self.currentIndexPath.item;
        vc.previewShowDeleteButton = self.previewShowDeleteButton;
        vc.showBottomPageControl = self.previewShowBottomPageControl;
        vc.photoView = self;
        vc.modalPresentationStyle = UIModalPresentationOverFullScreen;
...
- (void)directGoPhotoViewController {
    UIViewController *presentingVC = [self hx_viewController];
    ...
    [PPPermissionHelper requestPhotoLibraryPermissionFromViewController:presentingVC
        ...
        HXCustomNavigationController *nav = [[HXCustomNavigationController alloc] initWithManager:weakSelf.manager delegate:weakSelf];
...
- (void)goCameraViewController {
    ...
    HXWeakSelf
    [PPPermissionHelper requestCameraPermissionFromViewController:[self hx_viewController]
                                                       completion:^(BOOL granted) {
        if (!granted) return;
        if (!weakSelf) return;
