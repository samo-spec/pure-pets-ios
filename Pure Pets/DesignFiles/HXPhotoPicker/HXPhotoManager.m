...
- (HXPhotoModel *)photoModelWithAsset:(PHAsset *)asset {
    ...
    [[PHImageManager defaultManager] requestImageDataAndOrientationForAsset:asset
        ...
        isInCloud = [info[PHImageResultIsInCloudKey] boolValue] || (imageData == nil);
    }];

    // use isInCloud
    
    
    // It may not be accurate after ios13, but it doesn't matter.
     // Because iCloud judgment has been made when obtaining.  This is just for easy identification when displaying
     if (isInCloud) {
