...
- (void)setupStyleSheet {
    ...
    CGFloat contentViewHeight = rowHeight * 7 + headerViewHeight;
    ...
    [UIView animateWithDuration:0.2 animations:^{
        if (self.isShadeBackground) {
            self.dismissView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.4];
        }
        self.contentView.frame = contentViewFrame;
        self.headerView.frame = headerViewFrame;
        self.datePicker.frame = datePickerFrame;
...
- (UIColor *)headerViewBackgroundColor {
    if (!_headerViewBackgroundColor) {
        _headerViewBackgroundColor = [UIColor blueColor];
    }
