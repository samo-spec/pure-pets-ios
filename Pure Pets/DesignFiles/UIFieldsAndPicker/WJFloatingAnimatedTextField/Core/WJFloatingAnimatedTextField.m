...
- (void)commonInit {
    NSAssert(!_config.leftLabelString || !_config.leftIconImage, @"Pictures and text cannot coexist on the same side");
     NSAssert(!_config.rightLabelString || !_config.rightIconImage, @"Pictures and text cannot coexist on the same side");
...
- (void)addFloatingLabel {
    ...
    _floatingLabel.text = _config.floatingText ?: @"placeholder text";
...
- (void)simulateCallShouldChangeCharactersMethodWithText:(NSString *)text {
    //Simulation trigger shouldChangeCharactersInRange Even if the proxy method returns YES, it will not add the last character, so when simulating, you should simulate inputting a character in the character set.
    if (text) {
