...
- (void)viewDidLayoutSubviews
{
    ...
    if (@available(iOS 11.0, *)) { // iOS 11
        ...
        titleView.py_y = self.view.py_width > self.view.py_height ? 10 : 20;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self pp_navBarApplyBase:PPNavBarBaseLayoutAuto button:self.cancelButton title:@"" showBack:NO];
    ...
    self.baseSearchTableView.semanticContentAttribute = GM.setSemantic;
}
...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        ...
        [baseSearchTableView setSemanticContentAttribute:UISemanticContentAttributeForceRightToLeft];
...
- (UIButton *)emptyButton
{
    if (!_emptyButton) {
        ...
        emptyButton.titleLabel.font = [GM MidFontWithSize:16];
...
{
    if (!_colorPol) {
        NSArray *colorStrPol = @[@"B21B48", @"811334", @"C32148", @"D03D56"];
...
- (void)setup
{
    self.view.backgroundColor = PPBackgroundColorForIOS26([UIColor secondarySystemBackgroundColor]);
    ...
    cancleButton.titleLabel.font = [GM MidFontWithSize:14];
    [cancleButton setImage:[UIImage systemImageNamed:@"multiply"] forState:UIControlStateNormal];
    ...
    backButton.titleLabel.font = [GM MidFontWithSize:14];
    ...
    self.searchBarCornerRadius = 15.0;
    ...
    // Or use system font with specific size
    searchTextField.font = [GM MidFontWithSize:14];
    ...
    emptySearchHistoryLabel.font = [GM MidFontWithSize:14];
...
- (void)setupHotSearchRectangleTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rectangleTagLabel.font = [GM MidFontWithSize:14];
        rectangleTagLabel.textColor = PYTextColor;
        rectangleTagLabel.backgroundColor = [UIColor colorWithHexString:@"#F3F3F3"];
...
- (void)setupHotSearchRankTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rankTag.font = [GM MidFontWithSize:14];
        ...
        rankTextLabel.font = [GM MidFontWithSize:14];
    ...
    switch (hotSearchStyle) {
        case PYHotSearchStyleColorfulTag:
            for (UILabel *tag in self.hotSearchTags) {
                ...
                tag.layer.borderColor = nil;
                tag.layer.borderWidth = 0.0;
                tag.backgroundColor = PYSEARCH_COLORPolRandomColor ;// PYSEARCH_COLORPolRandomColor ;//[GM appPrimaryColor]; //PYSEARCH_COLORPolRandomColor
                tag.layer.cornerRadius = tag.hx_h / 2;
