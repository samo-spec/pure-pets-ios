...
#define FTDefaultSelectedTextColor          [UIColor whiteColor]
#define FTDefaultCellSelectedBackgroundColor    [UIColor darkGrayColor]
#define FTDefaultSeparatorColor             [UIColor lightGrayColor]
...
#define FTDefaultShadowOpacity              0.1f
...
@property (weak , nonatomic) FTPopOverMenuView *submenuView;
@end
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // If using FTPopOverMenuModel, check for a submenu array.
    if ([object isKindOfClass:[FTPopOverMenuModel class]]) {
        FTPopOverMenuModel *menuModel = (FTPopOverMenuModel *)object;
        if (menuModel.subMenuArray && menuModel.subMenuArray.count > 0) {
            // Present submenu
            // You might calculate a new frame and arrow point based on the selected cell.
            // For example, get the cell frame relative to self (or window) and then adjust.
            CGRect cellFrame = [tableView rectForRowAtIndexPath:indexPath];
            // Convert cell frame to the background view’s coordinate if needed:
            CGRect convertedFrame = [self convertRect:cellFrame toView:self.superview];
            
            // Create a new FTPopOverMenuView instance for the submenu.
            weakSelf.submenuView = [[FTPopOverMenuView alloc] initWithFrame:CGRectMake(0, 0, self.config.menuWidth, self.config.menuRowHeight * menuModel.subMenuArray.count + self.menuArrowHeight)];
            
            // Show the submenu – you can adjust the arrow point based on your design.
            // Here, we assume the submenu appears to the right of the selected cell.
            CGPoint submenuArrowPoint = CGPointMake(CGRectGetMaxX(convertedFrame), CGRectGetMidY(convertedFrame));
            
            // Note: For submenu, you may pass the submenu model titles as an array.
            // Here, we assume that your _menuStringArray can also be an array of FTPopOverMenuModel.
            NSMutableArray *subMenuTitles = [NSMutableArray array];
            NSMutableArray *subMenuImages = [NSMutableArray array];
            for (FTPopOverMenuModel *subModel in menuModel.subMenuArray) {
                [subMenuTitles addObject:subModel.title];
                [subMenuImages addObject:subModel.image ?: @""];
            }
            
            // Here we use arrow direction Up as an example; adjust as needed.
            [_submenuView showWithFrame:weakSelf.submenuView.frame
                             anglePoint:submenuArrowPoint
                          withNameArray:subMenuTitles
                         imageNameArray:subMenuImages
                       shouldAutoScroll:NO
                                 config:self.config
                         arrowDirection:FTPopOverMenuArrowDirectionUp
                              doneBlock:^(NSInteger selectedIndex) {
                // Handle submenu selection here.
                NSLog(@"Submenu item %ld selected", (long)selectedIndex);
                // Optionally, call the parent doneBlock or perform further actions.
              //  [weakSelf.submenuView removeFromSuperview];
            }];
            
            // Add the submenu view to the background view (or another container)
            [self.superview addSubview:weakSelf.submenuView];
            return; // Exit so the parent menu does not call doneBlock.
        }
    }
    
    // If there is no submenu, update selection state if needed.
    if ([object isKindOfClass:[FTPopOverMenuModel class]]) {
