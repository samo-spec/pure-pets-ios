...
- (void)initDefaults {
    ...
    self.selectorViewColor = [GM appPrimaryColor];

    self.transitionStyle = LUNSegmentedControlTransitionStyleSlide;
    self.shapeStyle = LUNSegmentedControlShapeStyleLiquid;
    self.applyCornerRadiusToSelectorView = NO;
    self.gradientBounceColor = [GM appPrimaryColor];
    self.gradientBackVelocity = 1;
    self.shadowsEnabled = YES;
    self.shadowHideDuration = 1.8;
...
- (void)setupShadowForStateAtIndex:(NSInteger)index visible:(BOOL)visible animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        animation.toValue = @(visible ? 0.3 : 0.0);
        animation.fillMode = kCAFillModeForwards;
        animation.removedOnCompletion = NO;
        [self.shadowView.layer addAnimation:animation forKey:nil];
    } else {
        self.shadowView.layer.shadowOpacity = visible ? 0.3 : 0.0;
    }
    
    self.shadowView.layer.opacity = 0.3;
}
