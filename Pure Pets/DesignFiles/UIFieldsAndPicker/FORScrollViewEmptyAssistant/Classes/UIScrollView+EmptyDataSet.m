...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            //imageView
            [view.imageView.layer setShadowColor:[UIColor darkGrayColor].CGColor];
            [view.imageView.layer setShadowOffset:CGSizeMake(1, 1)];
            [view.imageView.layer setShadowOpacity:0.5];
            [view.imageView.layer setShadowRadius:2];
...
- (UIImageView *)imageView
{
    if (!_imageView)
    {
        ...
        _imageView.tintColor = [UIColor darkGrayColor];
...
- (UIButton *)button
{
    if (!_button)
    {
        ...
        _button.backgroundColor = GM.appPrimaryColor;
