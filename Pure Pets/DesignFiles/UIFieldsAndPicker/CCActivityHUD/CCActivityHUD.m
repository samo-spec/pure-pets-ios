...
- (void)show {
   // [self showWithType:CCActivityHUDIndicatorTypeDynamicArc]; // CCActivityHUDIndicatorTypeSpringBall
    [self showWithType:CCActivityHUDIndicatorTypeLeadingDots];
}
...
- (void)dismissWithText:(NSString *)text delay:(CGFloat)delay success:(BOOL)success {
    if (self.superview) {
        ...
        UIImageView *tickCrossImageView = [[UIImageView alloc] initWithFrame:CGRectMake(ViewFrameWidth/2-length/2 - 10, length/3 , length+19, length)];
        tickCrossImageView.image = [UIImage imageNamed:success?@"NewLogo.png":@"cross"];
        tickCrossImageView.image = [tickCrossImageView.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        tickCrossImageView.tintColor = [UIColor whiteColor];
