...
- (void)startDownloadOpeartionWithRequestTask:(JPResourceLoadingRequestWebTask *)requestTask
    ...
    JPDebugLog(@"Downloader Processing a request");
}
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    JPDebugLog(@"URLSession Received a response");
...
didCompleteWithError:(NSError *)error {
    JPDispatchSyncOnMainQueue(^{
        JPDebugLog(@"URLSession A request was completed, id is %ld, error is: %@", task.taskIdentifier, error);
        BOOL completeValid = self.runningTask && task.taskIdentifier == self.runningTask.dataTask.taskIdentifier;
        if(!completeValid){
            JPDebugLog(@"URLSession Completed a request that is not being requested, the id is: %d", task.taskIdentifier);
            return;
        }
